...
void chega(struct mundo *mundo, struct evento0 *item){
    ...
    if (esp){
        ...
        printf("ESPERA\n");
        fprio_insere(mundo->lista, e, EV_ESPERA, mundo->relogio);

    } else {

        printf("DESISTE\n");
        fprio_insere(mundo->lista, e, EV_DESISTE, mundo->relogio);
...
void espera(struct mundo *mundo, struct evento0 *item){
    ...
    fprio_insere(mundo->lista, e, EV_AVISA, mundo->relogio);
}
...
void desiste(struct mundo *mundo, struct evento0 *item){
    ...
    fprio_insere(mundo->lista, e, EV_VIAJA, mundo->relogio);
}
...
void avisa(struct mundo *mundo, struct evento2 *item){
    ...
    while(presentes < mundo->base[item->b].lotacao && tamFila > 0){
        ...
        fprio_insere(mundo->lista, e, EV_ENTRA, mundo->relogio);
    }
...
void entra(struct mundo *mundo, struct evento0 *item){
    ...
    fprio_insere(mundo->lista, e, EV_SAI, mundo->relogio + tpb);
}
...
void viaja(struct mundo *mundo, struct evento0 *item){
    ...
    fprio_insere(mundo->lista, e, EV_CHEGA, mundo->relogio + duracao);
}
...
void sai(struct mundo *mundo, struct evento0 *item){
    ...
    fprio_insere(mundo->lista, e, EV_VIAJA, mundo->relogio);
    fprio_insere(mundo->lista, v, EV_AVISA, mundo->relogio);
}
...
void missao(struct mundo *mundo, struct evento3 *item){
    ...
    if(tentativa){
        ...
        for(int i = 0; i < mundo->Nherois; ++i){
            ...
            if(cjto_pertence(mundo->base[id].presentes, i)){
                ...
                if(risco > rand()%31){
                    ...
                    fprio_insere(mundo->lista, e, EV_MORRE, mundo->relogio);
        ...
        fprio_insere(mundo->lista, e, EV_MISSAO, mundo->relogio + (24*60));
    }
